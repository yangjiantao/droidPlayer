...
public class MediaPlayerSeekView extends RelativeLayout {
    ...
    private TextView tv_seek_video_totel;
    ...
    protected void onFinishInflate() {
        ...
        tv_seek_video_totel = (TextView) findViewById(R.id.tv_seek_video_totel);
        mTvSeekCurrentPosition = (TextView) findViewById(R.id.tv_seek_current_position);
    }

    private void init(Context context) {
        this.mContext = context;
        LayoutInflater.from(mContext).inflate(R.layout.blue_media_player_seek_view, this);
    ...
    public void onGestureSeekChange(float deltaSeekDistance, float totalSeekDistance) {
        mTotalDeltaSeekDistance = mTotalDeltaSeekDistance + deltaSeekDistance;
        if (Math.abs(mTotalDeltaSeekDistance) >= mMinSeekDistance) {
            float deltaSeekPercentage = mTotalDeltaSeekDistance / totalSeekDistance;
            ...
            tv_seek_video_totel.setText("/" + MediaPlayerUtils.getVideoDisplayTime(mTotalPosition));
            ...
            mTvSeekCurrentPosition.setText(MediaPlayerUtils.getVideoDisplayTime(mSeekPosition));
    ...
    public void onGestureSeekBegin(long currentPosition, long totalPosition) {
        if (currentPosition < 0 || totalPosition < 0 || currentPosition > totalPosition)
            return;
